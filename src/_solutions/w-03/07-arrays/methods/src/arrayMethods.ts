 ...
 */
export function findFirstUserByRole(users: User[], role: string): User | undefined {
 ...
 */
export function getFreshBook(books: Book[]): Book {
    return books.reduce((x,y) => (x.year > y.year) ? x : y)
}
